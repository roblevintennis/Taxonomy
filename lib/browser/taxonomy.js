...
(function() {
    ...
    */
    ...
    var normalize = function(n) {
        ...
        if(typeof n.attr['data-slug'] === 'undefined' && typeof n.data === 'string') {
            var str = n.data.replace(/\s+/g, " "); // trim extra spaces between words
            // split on spaces and join with -, then lower case and remove non legal chars
            str = str.split(' ').join('-').toLowerCase().replace(/[^a-zA-Z0-9_\-]+/g,'');
            n.attr['data-slug'] = str.split(' ').join('-').toLowerCase();
        }
